...
假设有1亿个数据项，每个数据项值的范围在0至200之间，也就是有201个数；桶排序会先创建一个长度是201个数组，数组中每个位置都是容器，用于存储其对应的值；

例如容器1只存储数值为1的数据项，遍历所有数据项，将值填到容器对应的位置，读取时按照容器顺序依次取出数据项即可，这个是桶排序的核心思想。
...
例如桶1表示0-9范围的数，桶2表示10-19的数，通过范围的划分使桶容器支持更大的数据范围。
...
- 问题：一个long long型无序数组中，如果排好序后求相邻俩个数的最大差值，要求时间复杂度为O(N）
  ...
  3. 将无序数组中每个数据项装到对应的桶中，由于N个数据项装到N+1个桶中，必将产生至少一个空桶
  4. 在空桶左边非空桶的最大数与右边非空桶的最小数差值一定大于桶的区间大小，所以不需要关心桶内差值的大小，只需关心桶外部的情况
  5. 相邻非空桶的最大差值是桶区间大小的俩倍，所以遍历所有相邻非空桶的最大差值就能得到结果
...
由于将N个数据项填到N+1个桶中，必将产生一个空桶，空桶左边的非空桶中最大数与右边非空桶的最小数的差值一定大于桶区间大小，所以不需要关心桶中数的差值大小，只需要计算相邻非空桶数的差值即可。





相邻俩个桶的最小差值是1，最大差值是 区间*2-1；

有的桶有一个数据项，有的桶有多个数据项，在不需要关心桶中数的差值大小，只需要遍历相邻非空桶的差值就可以求出相邻俩个数的最大差值。
