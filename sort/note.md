...
- 问题：一个long long型无序数组中，如果排好序后求相邻俩个数的最大差值，要求时间复杂度为O(N）
  ...
  寻找无序数组中的最小值和最大值，确定数值的范围；

  准备N+1个桶，将数值的范围平均的分割在每个桶上，即每个桶表示数值的区间是平均且连续性的；

  将无序数组中每个数据项装到对应的桶中，由于N个数据项装到N+1个桶中，必将产生空的桶；

  由于每个桶表示数值范围是平均且连续性的，所以空桶左边的非空桶的最后一个数据跟空桶右边的非空桶的第一个数是有序相邻的，只要算出每个空桶相邻俩个数的最大差值，就能得到结果。
